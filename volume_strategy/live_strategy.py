...
# Strategy Parameters (MODERATE SETTINGS)
MIN_DELTA_BUY = 150
MIN_DELTA_SELL = -150
MIN_BUY_PERCENT = 60
MIN_SELL_PERCENT = 60
CONSECUTIVE_CANDLES = 2
TAKE_PROFIT_PCT = 0.6  # 0.3%
STOP_LOSS_PCT = 0.15   # 0.15%
MAX_HOLD_MINUTES = 30

# Trade Tracking
TRADES_FILE = "trades_1min.json"
...
# LOAD/SAVE TRADES
# -------------------------------------
# Use unified storage (JSON + MongoDB if configured)
import sys
import os
...
storage = TradeStorage(
    json_file=os.path.join(script_dir, TRADES_FILE),
    collection_name='volume_trades_1min'
)

def load_trades():
    """Load existing trades from storage"""
    return storage.load_trades()


def save_trade(trade_data):
    """Save trade to storage (JSON + MongoDB)"""
...
def print_current_market_state():
    ...
    print(f"\n{'='*80}")
    if HAS_COLOR:
        print(f"{Fore.CYAN}📊 LIVE MARKET MONITOR - {SYMBOL}{Style.RESET_ALL}")
    else:
        print(f"📊 LIVE MARKET MONITOR - {SYMBOL}")
    print(f"{'='*80}")
...
def on_open(ws):
    ...
    print(f"\n{'='*80}")
    if HAS_COLOR:
        print(f"{Fore.CYAN}🤖 VOLUME-PRICE STRATEGY - LIVE TRADING{Style.RESET_ALL}")
    else:
        print(f"🤖 VOLUME-PRICE STRATEGY - LIVE TRADING")
    print(f"{'='*80}")
